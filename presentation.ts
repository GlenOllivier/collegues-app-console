...
export default class Presentation {
  ...
  authentification() {
    let username: string;
    rl.question("Veuillez entrer un nom d'utilisateur : ")
      .then((saisie: string) => {
        username = saisie.trim();
        return rl.question("Veuillez entrer un mot de passe : "); 
      })
      .then((saisie:string) => {
        let password = saisie.trim();
        return service.login(username, password);
      })
      .then(rien => this.start())
      .catch(error => {
